...
export function useV1TradeLinkIfBetter(
  ...
  if (v1Trade) {
    if (isExactIn) {
      ...
      v1HasBetterTrade =
        !v2Trade || discountedV1Output.greaterThan(v2Trade.outputAmount);
    } else {
      ...
      v1HasBetterTrade =
        !v2Trade || inflatedV1Input.lessThan(v2Trade.inputAmount);
    }
  }

  return v1HasBetterTrade && input && output
    ? `https://v1.uniswap.exchange/swap?inputCurrency=${
        inputIsWETH ? "ETH" : input.address
      }&outputCurrency=${outputIsWETH ? "ETH" : output.address}`
    : undefined;
}
